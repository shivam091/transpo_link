...
class PurchaseOrderItems::UpdateReceivedQuantityService < ApplicationService
  ...
  def increment_received_quantity!
    ...
    new_total = purchase_order_item.received_quantity + quantity
    max_allowed = purchase_order_item.quantity

    final_quantity = [new_total, max_allowed].min

    if purchase_order_item.update(received_quantity: final_quantity)
      ServiceResponse.success(payload: { purchase_order_item: purchase_order_item })
    else
      ServiceResponse.error(payload: { purchase_order_item: purchase_order_item })
    end
  end
end
