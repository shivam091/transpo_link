...
class CreateTaxRates < ActiveRecord::Migration[8.0]
  ...
  def change
    create_table :tax_rates, id: :uuid do |t|
      ...
      t.check_constraint "tax_type IN (#{enum_values('tax_types')})", name: :check_tax_rates_tax_type_in_enum_values

      t.check_constraint "business_category IS NOT NULL", name: :check_tax_rates_business_category_presence
      t.check_constraint "business_category IN (#{enum_values('business_categories')})", name: :check_tax_rates_business_category_in_enum_values

      t.check_constraint "rate IS NOT NULL", name: :check_tax_rates_rate_presence
      t.check_constraint "rate BETWEEN 0.0 AND 100.0", name: :check_tax_rates_rate_range
