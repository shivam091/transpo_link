...
class CreateTaxDetails < ActiveRecord::Migration[8.0]
  def change
    create_table :tax_details, id: :uuid do |t|
      ...
      t.check_constraint "country IS NOT NULL OR tax_type NOT IN (#{country_specific_tax_types})", name: "check_tax_details_tax_type_requires_country"
    end
  end
  ...
  def country_specific_tax_types
    TaxDetail::COUNTRY_SPECIFIC_TAX_TYPES.map { |v| "'#{v}'" }.join(", ")
  end
end
