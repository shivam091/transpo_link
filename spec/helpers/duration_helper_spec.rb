...
RSpec.describe DurationHelper, type: :helper do
  describe "#prettify_seconds" do
    ...
    it "returns formatted duration for minutes and seconds" do
      expect(helper.prettify_seconds(90)).to eq("1 minute, 30 seconds")
    end

    it "returns formatted duration for hours, minutes, and seconds" do
      expect(helper.prettify_seconds(5_445)).to eq("1 hour, 30 minutes, 45 seconds")
    end

    it "returns formatted duration for days, hours, minutes, and seconds" do
      expect(helper.prettify_seconds(90_000)).to eq("1 day, 1 hour")
    end

    it "returns formatted duration for weeks and days" do
      expect(helper.prettify_seconds(1_296_000)).to eq("2 weeks, 1 day")
    end

    it "returns formatted duration for months, weeks, and days" do
      expect(helper.prettify_seconds(13_104_000)).to eq("4 months, 4 weeks, 1 day, 22 hours, 3 minutes, 36 seconds")
    end

    it "returns formatted duration for years, months, and weeks" do
      expect(helper.prettify_seconds(63_072_000)).to eq("1 year, 11 months, 4 weeks, 1 day, 22 hours, 50 minutes, 42 seconds")
    end
