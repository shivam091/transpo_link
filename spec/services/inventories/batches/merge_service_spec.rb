...
# spec/services/inventories/batches/merge_service_spec.rb

require "spec_helper"

RSpec.describe Inventories::Batches::MergeService, type: :service do
  let!(:unit) do
  ...
  let!(:batch) { create(:inventory_batch, quantity: 5, unit:) }

  subject(:service_response) { described_class.(batch, batch_attributes) }

  describe ".call" do
    context "when merge succeeds after unit conversion" do
      let(:batch_attributes) { {quantity: 2, unit_id: batch.unit_id} }

      it "updates the batch quantity by adding the given amount" do
        expect {
          service_response
        }.to change { batch.reload.quantity }.by(2)
      end
    ...
    context "when unit conversion raises an error" do
      ...
      let(:batch_attributes) { {quantity: 2, unit_id: source_unit.id} }

      before do
        allow(batch).to receive(:unit) { target_unit }
    ...
    context "when merge fails after successful conversion" do
      ...
      before do
        ...
        allow(batch).to receive(:merge_with!) { false }
      end

      it "does not change the batch quantity" do
        expect {
          service_response
        }.to not_change { batch.reload.quantity }
      end
