...
# spec/services/inventories/batches/upsert_service_spec.rb

require "spec_helper"

RSpec.describe Inventories::Batches::UpsertService, type: :service do
  ...
  describe ".call" do
    context "when batch_number and expiration_date are provided" do
      context "when the matching batch does not exist" do
        ...
        it "calls Inventories::Batches::CreateService" do
          expect(Inventories::Batches::CreateService)
            .to receive(:call)
            .with(inventory, batch_attributes)
            .and_call_original
      ...
      context "when a matching batch already exists" do
        ...
        it "calls Inventories::Batches::MergeService" do
          expect(Inventories::Batches::MergeService)
            .to receive(:call)
            .with(existing_batch, batch_attributes)
            .and_call_original
