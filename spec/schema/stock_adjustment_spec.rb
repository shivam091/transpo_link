...
RSpec.describe StockAdjustment, type: :model do
  describe "attributes" do
    ...
    it { is_expected.to have_db_column(:inventory_id).of_type(:uuid) }
    it { is_expected.to have_db_column(:adjustable_type).of_type(:string).with_options(null: false) }
    it { is_expected.to have_db_column(:adjustable_id).of_type(:uuid).with_options(null: false) }
    ...
    it { is_expected.to have_db_column(:adjustment_type).of_type(:enum) }
  ...
  describe "indexes" do
    it { is_expected.to have_db_index([:adjustable_type, :adjustable_id]) }
    it { is_expected.to have_db_index([:source_type, :source_id]) }
    it { is_expected.to have_db_index(:adjusted_at) }
    it { is_expected.to have_db_index(:adjustment_type) }
  ...
  describe "foreign keys" do
    it { is_expected.to have_foreign_key(:inventory_id).with_name(:fk_stock_adjustments_inventory_id_on_inventories).on_delete(:cascade) }
  ...
  describe "check constraints" do
    ...
    it { is_expected.to have_check_constraint(:check_stock_adjustments_adjustment_type_in_enum_values) }
    it { is_expected.to have_check_constraint(:check_stock_adjustments_adjustment_type_presence).with_expression("adjustment_type IS NOT NULL") }
