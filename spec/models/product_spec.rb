...
RSpec.describe Product, type: :model do
  ...
  describe "nested attributes" do
    it { is_expected.to accept_nested_attributes_for(:unit_conversions).allow_destroy(true) }
  ...
  describe "instance methods" do
    ...
    describe "#reject_unit_conversion?" do
      ...
      context "when creating unit conversions" do
        context "when valid attributes are provided" do
          it "creates a unit conversion" do
            expect {
              product.update(unit_conversions_attributes: {0 => {from_unit: "m", to_unit: "cm", conversion_rate: 100}})
            }.to change(UnitConversion, :count).by(1)
          end
        end
      ...
      context "when updating unit conversions" do
      ...
      context "when destroying unit conversions" do
        it "removes the unit conversion when _destroy is set to true" do
          expect {
            product.update(unit_conversions_attributes: {id: unit_conversion.id, _destroy: true})
          }.to change(UnitConversion, :count).by(-1)
        end
      end
    end
