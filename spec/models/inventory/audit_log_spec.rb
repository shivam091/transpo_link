...
# spec/models/inventory/audit_log_spec.rb
...
RSpec.describe Inventory::AuditLog, type: :model do
  ...
  describe "attributes, indexes, foreign keys, and check constraints" do
    ...
    it { is_expected.to have_db_column(:movement_id).of_type(:uuid).with_options(null: true) }
    ...
    it { is_expected.to have_db_index([:inventory_id, :type]) }
    it { is_expected.to have_db_index(:inventory_id) }
    it { is_expected.to have_db_index(:movement_id) }
    ...
    it { is_expected.to have_foreign_key(:movement_id).with_name(:fk_inventory_audit_logs_movement_id_on_inventory_movements).on_delete(:cascade) }
    it { is_expected.to have_foreign_key(:user_id).with_name(:fk_inventory_audit_logs_user_id_on_users).on_delete(:nullify) }

    it { is_expected.to have_check_constraint(:check_inventory_audit_logs_type_presence).with_expression("type IS NOT NULL") }
  ...
  describe "associations" do
    it { is_expected.to belong_to(:inventory).inverse_of(:audit_logs) }
    it { is_expected.to belong_to(:movement).class_name("Inventory::Movement").inverse_of(:audit_logs).optional }
