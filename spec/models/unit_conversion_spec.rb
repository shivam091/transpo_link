  ...
  describe "class methods" do
    describe ".convert" do
      ...
      context "when source and target units are the same (Unit objects)" do
        ...
        it "returns the same quantity as float" do
      ...
      context "when conversion exists (using Unit objects)" do
        let!(:conversion) { create(:kilogramme_gramme_conversion, source_unit: source_unit, target_unit: target_unit) }

        let(:result) { described_class.convert(source_unit, target_unit, 2) }

        it "returns the converted quantity as float" do
