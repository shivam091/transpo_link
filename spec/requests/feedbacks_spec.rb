...
RSpec.describe "Feedbacks", type: :request do
  ...
  context "when user is not signed in" do
    describe "GET /feedbacks" do
      subject { get feedbacks_path }

      it { is_expected.to require_sign_in }
    end

    describe "GET /feedbacks/new" do
      subject { get new_product_feedback_path(product) }

      it { is_expected.to require_sign_in }
    end

    describe "POST /feedbacks" do
      subject { post product_feedbacks_path(product) }

      it { is_expected.to require_sign_in }
    end

    describe "PUT|PATCH /feedbacks/:id/mark-as-read" do
      subject { put mark_as_read_feedback_path(unread_feedback) }

      it { is_expected.to require_sign_in }
    end

    describe "GET /feedbacks/:id" do
      subject { get feedback_path(unread_feedback) }
      ...
      describe "PUT|PATCH /feedbacks/:id/mark-as-read" do
        context "when unread feedback" do
          it "marks feedback as read and redirects" do
            ...
            expect(unread_feedback.reload.is_unread?).to be_falsy
            expect(response).to redirect_to(root_path)
