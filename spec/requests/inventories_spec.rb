...
RSpec.describe "Inventories", type: :request do
  ...
  context "when user is not signed in" do
    ...
    describe "POST /inventories" do
      subject { post inventories_path }

      it { is_expected.to require_sign_in }
    end

    describe "GET /inventories/:id/edit" do
      subject { get edit_inventory_path(inventory) }

      it { is_expected.to require_sign_in }
    end

    describe "PUT|PATCH /inventories/:id" do
      subject { put inventory_path(inventory) }

      it { is_expected.to require_sign_in }
    end

    describe "GET /inventories/:id" do
      subject { get inventory_path(inventory) }
    ...
    describe "PUT|PATCH /inventories/:id" do
      context "when provided attributes are valid" do
        it "updates the inventory and redirects" do
          ...
          expect(inventory.reload.inventory_unit).to eq("mg")
          expect(response).to redirect_to(inventories_path)
