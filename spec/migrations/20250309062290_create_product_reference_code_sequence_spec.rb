# -*- encoding: utf-8 -*-
# -*- frozen_string_literal: true -*-
# -*- warn_indent: true -*-

# spec/migrations/20250309062290_create_product_reference_code_sequence_spec.rb

require "spec_helper"
require_migration!

RSpec.describe CreateProductReferenceCodeSequence, type: :migration do
  let(:sequence_name) { "product_reference_code_seq" }

  describe ".up" do
    before do
      run_migration(:down)
      run_migration(:up)
    end

    it "creates the sequence" do
      expect(sequence_exists?(sequence_name)).to be_truthy
    end

    it "sets ownership of the sequence" do
      expect(sequence_ownership(sequence_name)).to eq(
        "table_name" => "products",
        "column_name" => "reference_code"
      )
    end
  end

  describe ".down" do
    before { run_migration(:down) }

    it "drops the sequence" do
      expect(sequence_exists?(sequence_name)).to be_falsy
    end
  end
end
