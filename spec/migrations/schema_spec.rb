...
RSpec.describe ActiveRecord::Schema, type: :migration do
  ...
  let(:latest_migration_timestamp) { all_migrations.max }
  ...
  it "the schema_migrations table contains all schema versions" do
    versions = connection.select_values("SELECT version FROM schema_migrations ORDER BY version").map(&:to_i)
